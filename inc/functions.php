...
*/
    ...
    If (isset($cron)){
        ...
        $tweet = json_decode($twitter->setGetfield('query='.$TwitterSpecialTag.'&max_results=10&expansions=author_id&tweet.fields=created_at,referenced_tweets&place.fields=name&user.fields=username')->buildOauth('https://api.twitter.com/2/tweets/search/recent', 'GET')->performRequest());
            ...
            // we check if the user has an account with more than 30 days and if it follows more then 100 users and is followed by at leat 100 users, because of fake accounts and abusers
                ...
                if (time() > strtotime($tweetUser->data->created_at . ' +30 day') and $tweetUser->data->public_metrics->followers_count > 69 and $tweetUser->data->public_metrics->following_count > 69){
                    ...
                    if ($type != "retweeted"){
                        ...
                        $wallet = trim($muchdoge[0]);

                        // We validate the Shib Wallet Address to make sure is correct formated
                        if (preg_match('/^D{1}[5-9A-HJ-NP-U]{1}[1-9A-HJ-NP-Za-km-z]{32}+$/', $wallet) != 1){
                            $amount = 0;
                        }else{
                          // validate the Twitter username to
                          if (preg_match('/^[\w]+$/', $to) == 1){
                            ...
                            $tweetUserTo = json_decode($twitter->setGetfield('user.fields=created_at,public_metrics')->buildOauth('https://api.twitter.com/2/users/by/username/'.$to, 'GET')->performRequest());
                            if (time() < strtotime($tweetUserTo->data->created_at . ' +30 day') and $tweetUserTo->data->public_metrics->followers_count < 69 and $tweetUserTo->data->public_metrics->following_count < 69){
                                      $amount = 0;
                        ...
                        if (isset($tweet->data[$i]->id)){
                            ...
                            if ($amount > 0){
                                  ...
                                  if ($db->fetch()){
                                      ...
                                      $transaction_id = $DogePHPbridgeCommand->sendtoaddress($wallet, $amount, "TwitterID:".$tweet->data[$i]->id, $to." From:".$username);
        ...
        $totalTips = round($DogePHPbridgeCommand->getbalance("*"),2);
    };
?>